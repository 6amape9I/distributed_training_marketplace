...
class OrchestratorClient:
    ...
    def claim_task(self, *, evaluator_node_id: str) -> EvaluationTaskRecord | None:
        response = self._client.post("/evaluator/tasks/claim", json={"evaluator_node_id": evaluator_node_id})
        response.raise_for_status()
        payload = response.json()
        if payload is None:
            return None
        return EvaluationTaskRecord.model_validate(payload)

    def start_task(self, *, evaluation_task_id: str, evaluator_node_id: str) -> EvaluationTaskRecord:
        response = self._client.post(
            f"/evaluator/tasks/{evaluation_task_id}/start",
            json={"evaluator_node_id": evaluator_node_id},
        )
        response.raise_for_status()
        return EvaluationTaskRecord.model_validate(response.json())
    ...
    def complete_task(
        ...
        *,
        ...
        response = self._client.post(
            f"/evaluator/tasks/{evaluation_task_id}/complete",
            json={
    ...
    def fail_task(
        ...
        *,
        ...
        response = self._client.post(
            f"/evaluator/tasks/{evaluation_task_id}/fail",
            json={"evaluator_node_id": evaluator_node_id, "failure_reason": failure_reason},
        )
        response.raise_for_status()
        return EvaluationTaskRecord.model_validate(response.json())
